...
import { Sparkles, Leaf, ShieldAlert, Plus, Check, ChevronDown, ChevronUp, Star } from 'lucide-react';
...
export default function MenuItemCard({ item, lang, onAddToTray }: MenuItemCardProps) {
  ...
  return (
    <motion.div 
      layout
      className="bg-white rounded-2xl overflow-hidden border border-[#3D2B1F]/10 shadow-xs hover:shadow-md transition-shadow duration-300 flex flex-col h-full justify-between"
    ...
    >
          ...
          {hasOptions && (
            <div className="mt-3 pt-2 border-t border-stone-100">
              <button
                ...
                className="w-full flex items-center justify-between text-stone-500 hover:text-[#3D2B1F] text-[11px] font-bold py-1 px-1.5 rounded-lg hover:bg-[#F2EDE7]/40 transition-colors cursor-pointer"
                id={`config-btn-${item.id}`}
              >
                <span className="flex items-center gap-1">
                  ⚙️ {lang === 'az' ? 'Seçimləri fərdiləşdir' : 'Customize options'}
                </span>
      ...
      {/* 🔴 BÜTÜN MƏHSULLARDA SƏBƏTƏ ƏLAVƏ ET DÜYMƏSİ (Əlavə Olundu) */}
      <div className="p-4 pt-0 mt-3">
        <button
          type="button"
          onClick={handleAdd}
          disabled={addedAnimation}
          className={`w-full py-2.5 px-3 rounded-xl font-bold text-xs flex items-center justify-between transition-all duration-300 shadow-sm active:scale-[0.98] cursor-pointer ${
            addedAnimation
              ? 'bg-emerald-600 text-white'
              : 'bg-[#3D2B1F] hover:bg-[#2A1E15] text-[#FCF9F5]'
          }`}
        >
          <span className="flex items-center gap-1.5">
            {addedAnimation ? (
              <>
                ...
                <span>{lang === 'az' ? 'Səbətə əlavə olundu!' : 'Added to tray!'}</span>
              </>
            ) : (
              <>
                <Plus className="w-4 h-4 text-amber-400" />
                <span>{lang === 'az' ? 'Səbətə Əlavə Et' : 'Add to Tray'}</span>
              </>
            )}
          </span>

          {/* Əgər süd və ya ölçü dəyişsə, yekun hesablanmış qiyməti göstərir */}
          <span className="font-mono text-xs font-black text-amber-300 bg-black/20 px-2 py-0.5 rounded-md">
